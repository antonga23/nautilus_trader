...
from nautilus_trader.config import CacheDatabaseConfig, CacheConfig, LiveDataEngineConfig, LiveExecEngineConfig, \
    LiveRiskEngineConfig, LiveExecClientConfig
...
async def main(market_filters: Optional[dict]):
    """
    A trading node for Cloudbet.

    Parameters
    ----------
    market_filters : Optional[dict]
    The venue specific market filters.
    example:
        filters = {
            'sport_key': 'tennis',
    ...
    """
    # TODO: fix type for filters in cloudbet schema
    ...
    trading_config = TradingNodeConfig(
        ...
        cache_database=CacheDatabaseConfig(type="redis", flush=True),  # TODO: replace with redis
        data_engine=LiveDataEngineConfig(
            debug=True,
            qsize=10_000,  # TODO: set qsize based on expected number of Instruments
        ),
        ...
        risk_engine=LiveRiskEngineConfig(
            ...
            max_notional_per_order={},  # TODO: this is calculated on the fly from max stake per Selection/InstrumentId
            debug=True
        ),
        data_clients={
    ...
    strategies = [
        BettingMarketMaker(
            ...
            max_size=Decimal(0),# TODO: pass from RISK ENGINE or instrument max size
            trigger_min_size=Decimal(0), # TODO: pass from RISK ENGINE or instrument min size
            trigger_min_profit=float(0.5), # TODO: pass from RISK ENGINE
