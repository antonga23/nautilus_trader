...
class CloudbetError(Exception):
    """
    The base class for all `Cloudbet` specific errors.
    """


class CloudbetAPIError(CloudbetError):
    """
    Represents a `Cloudbet` API specific error.
    """
    ...
    def __str__(self) -> str:
        # return f"CloudbetAPIError(code='{self.code}', message='{self.message}', kind='{self.kind}', reason='{self.reason}')"
        return f"CloudbetAPIError(code='{self.code}', message='{self.message}')"
