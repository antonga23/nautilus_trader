...
from tests.integration_tests.adapters.cloudbet.test_kit import (
    CloudbetTestStubs,
    test_api_key,
    test_api_url,
    CloudbetResponses,
...
pytestmark = pytest.mark.skipif(
    os.getenv("RUN_LIVE_CLOUDBET_TESTS") != "1",
    reason="Requires live Cloudbet API access and is excluded from default hermetic validation.",
)
...
class TestCloudbetClient:
    ...
    async def test_client_get_events_for_sports(self):
        ...
        result = await self.client.get_events_for_sport(
            sport_key, current_timestamp, timestamp_48h, limit=5
        )
    ...
    async def test_get_events_for_sport_executed_asynchronously(self):
        ...
        self.client.get_events_for_sport = AsyncMock(
        ...
        self.client.get_events_for_sport.assert_has_calls(
            ...
            any_order=True,
        )  # any_order=True means we don't care about the order of the calls
    ...
    async def test_get_events_for_sport_market_filter(self):
        ...
        result = await self.client.get_events_for_sport(
            sport_key, current_timestamp, timestamp_48h, limit=5, markets=markets
        )
        expected = CloudbetResponses.get_events_for_sport()
        assert type(result) == type(expected)
    ...
    async def test_get_fixture_success(self):
        ...
        sports = ["soccer", "tennis", "baseball", "basketball"]
        sport_key = random.choice(sports)
        result = await self.client.get_fixtures(
            sport_key, current_timestamp, timestamp_48h, limit=100
        )
    ...
    async def test_get_event_success(self):
        ...
        sports = ["soccer"]
        sport_key = random.choice(sports)
        fixtures: GetFixturesResponse = await self.client.get_fixtures(
            sport_key, current_timestamp, timestamp_48h, limit=100
        )
    ...
    async def test_get_latest_odds(self, mock_get_latest_odds, mock_get_events_for_sport):
        ...
        market_url = (
            selection.market_name + "/" + selection.outcome + "?" + selection.params
            if selection.params is not None
            else selection.market_name + "/" + selection.outcome
        )
        # await self.client.connect()
    ...
    async def test_place_bet_accepted(
        self, mock_place_bet, mock_get_events_for_sport, cloudbet_client
    ):
        """
        ...
        assert isinstance(selection, Selection), (
            f"Expected object of type Selection. received {selection} "
        )
        market_url = (
            selection.market_name + "/" + selection.outcome + "?" + selection.params
            if selection.params is not None
        ...
        result = await cloudbet_client.place_bets(
            selection.event_id,
            market_url,
            selection.price,
            selection.side,
            selection.min_stake,
            currency="PLAY_EUR",
        )  # use PLAY_EUR for testing
        assert isinstance(result, GetBetResponse)
        assert result.status is BetStatus.ACCEPTED, (
            f"Expected ACCEPTED BetStatus, instead got: {result.status}"
        )
    ...
    async def test_place_bet_mock_required_parameters(self, mock_cloudbet_post, cloudbet_client):
        ...
        result = await cloudbet_client.place_bets(
            success_bet.event_id,
            market_url=success_bet.market_url,
            price=success_bet.price,
            side=success_bet.side.value,
            stake=success_bet.stake,
            reference_id=success_bet.reference_id,
            currency=success_bet.currency,
        )
        ...
        mock_cloudbet_post.assert_called_once_with(
            url=f"{cloudbet_client._api_url}/v3/bets/place",
    ...
    @patch.object(CloudbetClient, "get_events_for_sport", new_callable=AsyncMock)
    # @patch.object(CloudbetClient, 'place_bets', new_callable=AsyncMock)
    async def test_fail_to_place_bet_raises_exception(
        self, mock_get_events_for_sport, mock_cloudbet_post, cloudbet_client
    ):
        ...
        with pytest.raises(CloudbetAPIError):
            await cloudbet_client.place_bets(
                event_id,
                market_url,
                selection.price,
                selection.side,
    ...
    async def test_retrieve_bet_history_required_params(self, mock_cloudbet_get, cloudbet_client):
        ...
        limit = 100
        offset = 0
        query_params = {"fromDate": from_date, "toDate": to_date, "limit": limit, "offset": offset}
        ...
        mock_cloudbet_get.assert_called_once_with(
            url=f"{cloudbet_client._api_url}/v4/bets/history",
            params=query_params,
            headers=cloudbet_client.headers,
        )
    ...
    @pytest.mark.asyncio
    @patch.object(CloudbetClient, "get")
    async def test_retrieve_bet_history_offset_or_limit_invalid(
        self, mock_cloudbet_get, cloudbet_client
    ):
        # Arrange
        from_date = "2023-09-11T00:00:00Z"  # hardcoded since we know these dates have bets placed
        to_date = "2023-09-20T23:59:59Z"  # hardcoded since we know these dates have bets placed
        ...
        with pytest.raises(ValueError):
            ...
            mock_cloudbet_get.assert_called_once_with(
                ...
                params=query_params,
                headers=cloudbet_client.headers,
            )  # important sanity check otherwise Error could be thrown for multiple reasons

    @pytest.mark.asyncio
    @patch.object(CloudbetClient, "get")
    async def test_retrieve_bet_history_exception_malformed_date(
        self, mock_cloudbet_get, cloudbet_client
    ):
        # Arrange
        response_data: bytes = json.dumps(
            "parsing time 2023/09/11 as 2006-01-02T15:04:05Z07:00: cannot parse /09/11 as -"
        ).encode("utf-8")  # encode to binary
        GenericObject = namedtuple("GenericObject", ["status", "data"])
    ...
    async def test_get_bet_status_valid_reference_id(self, mock_cloudbet_get, cloudbet_client):
        ...
        assert isinstance(result, GetBetResponse)
        mock_cloudbet_get.assert_called_once_with(
            url=f"{cloudbet_client._api_url}/v3/bets/{bet_status.reference_id}/status",
            headers=cloudbet_client.headers,
        )

    @pytest.mark.asyncio
    @patch.object(CloudbetClient, "get")
    async def test_raises_cloudbet_api_error_for_invalid_reference_id(
        self, mock_cloudbet_get, cloudbet_client
    ):
        # Set up the mock response
        ...
        mock_cloudbet_get.assert_called_once_with(
            url=f"{cloudbet_client._api_url}/v3/bets/{reference_id}/status",
            headers=cloudbet_client.headers,
        )
    ...
    async def test_get_account_currencies_success(self, mock_cloudbet_get, cloudbet_client):
        ...
        mock_cloudbet_get.assert_called_once_with(
            url=f"{cloudbet_client._api_url}/v1/account/currencies", headers=cloudbet_client.headers
        )

    @pytest.mark.asyncio
    @patch.object(CloudbetClient, "get")
    async def test_get_account_currencies_exception_handling(
        self, mock_cloudbet_get, cloudbet_client
    ):
        # Set up the mock response
    ...
    async def test_get_balances_success(self, mock_cloudbet_get, cloudbet_client):
        ...
        mock_cloudbet_get.assert_called_once_with(
            url=f"{cloudbet_client._api_url}/v1/account/currencies/{currency}/balance",
            headers=cloudbet_client.headers,
        )
