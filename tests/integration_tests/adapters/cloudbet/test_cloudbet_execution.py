...
class TestCloudbetExecutionClient:
    ...
    async def test_exec_client_fixture(self, exec_client):
        ...
        assert isinstance(exec_client, CloudbetLiveExecutionClient), f"Expected exec client to be CloudbetLiveExecutionClient, got {type(exec_client)}"
    ...
    # stream_connect is the mock object for teh above patch
    ...
    # async def test_request_instrument_loaded_instrument(self, mock_get_latest_odds, data_client, data_engine,
    #                                                     instruments):
    ...
    #     assert data_client._cache.load_instrument(loaded_instrument.id) == instruments[
    #         0], f"Expected {loaded_instrument.id} to be in cache, got {data_client._cache.load_instrument(loaded_instrument.id)}"
